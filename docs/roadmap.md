...
**Left open, deliberately, and where it is written down.** Uploaded media is not readable in a real deploy: nothing signs a GET, so the gallery and `/documentos` point `<img src>`/`<a href>` straight at a bucket whose public access block answers 403. The demo hides this because its seeded media points at picsum.photos. That is also why A9's `ContentDisposition: attachment` was not done — there is no read path to attach it to, and the fix is a presigned GET carrying `ResponseContentDisposition`, which is a feature rather than bucket hardening. The reasoning sits next to the bucket in `template.yaml`. The twelve React Compiler warnings are the other open item, in `eslint.config.mjs`.

**F9.6 — `fix/performance-a11y-pass`** — Core Web Vitals, accessibility audit (axe) on Phase 8 pages. Also the natural home for the two items F9.5 left open above. Commit: `fix: performance and accessibility pass across new pages`

**F9.7 — `docs/portfolio-readme-demo`** — Root README with demo links (Vercel + AWS API), screenshots, final architecture diagram. Commit: `docs: update root README with production links and architecture diagram`
...
| §2    | Non-functional                   | F6.3 (security/CORS), F7.5 (e2e testing), F9.2 (SEO), F9.3 (observability), F9.4 (OWASP), F9.5 (OWASP follow-ups + next@16), F9.6 (performance/a11y) |
