...
**Rate limiting stays in per-instance memory.** `@nestjs/throttler` counts in each Lambda instance's own memory with no shared store, so the API's real ceiling is `APP_THROTTLE_LIMIT × concurrent instances`. F9.4 turned that from "unbounded" into a number by capping concurrency (`ReservedConcurrentExecutions`, SAM `MaxConcurrency`); the ceiling is therefore known, but it is still a multiple of the configured limit, and a caller spread across instances sees the higher number. Which number depends on where the cap comes from: `MaxConcurrency` defaults to 0 — reserve nothing — because Lambda refuses any reservation that would leave an account with less than 10 unreserved concurrency, and a new account's whole quota is 10. On such an account the account quota is the ceiling, so the limit is 1000 req/min rather than the 500 a `MaxConcurrency` of 5 would give. The control does not disappear when the reservation cannot be made; it moves to the account, and the arithmetic changes with it.
